...
import oss.s3.transfer
import oss.session
from tests import unique_id, unittest

urlopen = six.moves.urllib.request.urlopen
LOG = logging.getLogger('oss.tests.integration')
...
def random_bucket_name(prefix='oss-transfer', num_chars=10):
...
def clear_out_bucket(bucket, region, delete_bucket=False):
    s3 = oss.client('s3', region_name=region)
...
class TestS3Resource(unittest.TestCase):
    def setUp(self):
        ...
        self.session = oss.session.Session(region_name=self.region)
...
class TestS3Transfers(unittest.TestCase):
    """Tests for the high level oss.s3.transfer module."""
    ...
    def create_s3_transfer(self, config=None):
        return oss.s3.transfer.S3Transfer(self.client, config=config)
    ...
    def test_upload_fileobj_progress(self):
        ...
        chunksize = 5 * (1024**2)
        config = oss.s3.transfer.TransferConfig(
    ...
    def test_callback_called_once_with_sigv4(self):
        ...
        transfer = oss.s3.transfer.S3Transfer(client)
        filename = self.files.create_file_with_size(
