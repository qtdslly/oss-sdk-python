...
import oss
from oss.docs.service import ServiceDocumenter
...
@pytest.fixture
def boto3_session():
    return oss.Session(region_name='us-east-1')


def all_services():
    session = oss.Session(region_name='us-east-1')
...
def test_documentation(
    ...
    client = oss.client(service_name, 'us-east-1')
    ...
    if service_name in available_resources:
        ...
        resource = oss.resource(service_name, 'us-east-1')
...
def _assert_has_client_documentation(generated_docs, service_name, client):
    ...
    ref_lines = [
        ...
        '    import oss',
        '    client = oss.client(\'%s\')' % service_name,
...
def _assert_has_resource_documentation(generated_docs, service_name, resource):
    ref_lines = [
        ...
        '    import oss',
        f'    {service_name} = oss.resource(\'{service_name}\')',
    ]
    _assert_contains_lines_in_order(ref_lines, generated_docs)
