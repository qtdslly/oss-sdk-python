...
import oss
import oss.utils
from oss.exceptions import ResourceNotExistsError, UnknownAPIVersionError
...
class Session:
    ...
    def __init__(
        ...
        if self._session.user_agent_name == 'Botocore':
            ...
            self._session.user_agent_version = oss.__version__
    ...
    def client(
        ...
        :param access_key_id: The access key to use when creating
    ...
    def resource(
        ...
        :return: Subclass of :py:class:`~oss.resources.base.ServiceResource`
        """
        try:
        ...
        if api_version is None:
            ...
            # to track this api_version in oss in order to ensure
            # we're pairing a resource model with a client model
            # of the same API version.  It's possible for the latest
            # API version of a resource model in oss to not be
        ...
        service_context = oss.utils.ServiceContext(
            service_name=service_name,
            service_model=service_model,
            resource_json_definitions=resource_model['resources'],
            service_waiter_model=oss.utils.LazyLoadedWaiterModel(
    ...
    def _register_default_handlers(self):
        ...
        self._session.register(
            ...
            oss.utils.lazy_call(
                'oss.s3.inject.inject_s3_transfer_methods'
            ),
        )
        self._session.register(
            'creating-resource-class.s3.Bucket',
            oss.utils.lazy_call('oss.s3.inject.inject_bucket_methods'),
        )
        self._session.register(
            'creating-resource-class.s3.Object',
            oss.utils.lazy_call('oss.s3.inject.inject_object_methods'),
        )
        self._session.register(
            'creating-resource-class.s3.ObjectSummary',
            oss.utils.lazy_call(
                'oss.s3.inject.inject_object_summary_methods'
            ),
        )
        ...
        self._session.register(
            ...
            oss.utils.lazy_call(
                'oss.dynamodb.transform.register_high_level_interface'
            ),
        ...
        self._session.register(
            ...
            oss.utils.lazy_call(
                'oss.dynamodb.table.register_table_methods'
            ),
        ...
        self._session.register(
            ...
            oss.utils.lazy_call('oss.ec2.createtags.inject_create_tags'),
        )

        self._session.register(
            'creating-resource-class.ec2.Instance',
            oss.utils.lazy_call(
                'oss.ec2.deletetags.inject_delete_tags',
