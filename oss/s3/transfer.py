...
.. code-block:: python
    ...
    client = oss.client('s3', 'us-west-2')
...
.. code-block:: python
    ...
    transfer = S3Transfer(oss.client('s3', 'us-west-2'))
...
from oss.exceptions import RetriesExceededError, S3UploadFailedError
...
class S3Transfer:
    ...
    def __init__(self, client=None, config=None, osutil=None, manager=None):
        if not client and not manager:
            raise ValueError(
                'Either a oss.Client or s3transfer.manager.TransferManager '
                'must be provided'
            )
    ...
    def download_file(
        ...
        # exceeded we need to throw the same error from oss instead of
        # s3transfer's built in RetriesExceededError as current users are
        # catching the oss one instead of the s3transfer exception to do
        # their own retries.
